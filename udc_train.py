...
tf.flags.DEFINE_integer("eval_every", 6000, "Evaluate after this many train steps")
...
def main(unused_argv):
  ...
  class EvaluationHooks(tf.contrib.learn.monitors.EveryN):
    def every_n_step_end(self,step,outputs):
      self._estimator.evaluate(input_fn=input_fn_eval,
                              metrics=eval_metrics,
                              steps=None)

  eval_monitor = EvaluationHooks(every_n_steps=FLAGS.eval_every)
  #fit model
  estimator.fit(input_fn=input_fn_train, steps=FLAGS.eval_every )#monitors=[eval_monitor])

  #score accuracy
  pred = estimator.evaluate(input_fn=input_fn_eval,steps=1)
  print (pred)
  print ("Loss: %s"%pred["loss"])
  #print ("RMS: %s"%pred["rmse"])
